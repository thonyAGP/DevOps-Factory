...
describe('compliance-report', () => {
  ...
  describe('Summary Aggregation', () => {
    ...
    it('should calculate average compliance score', () => {
      ...
      const avg = Math.round(repos.reduce((s, r) => s + (r as any).score, 0) / repos.length);
...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      const repos: { mergedPRs?: unknown[]; score?: number }[] = [];
  ...
  describe('Security Findings Aggregation', () => {
    it('should aggregate findings by severity', () => {
      type SecurityFinding = { repo: string; type: string; severity: string; count: number; lastScan: string; };
    ...
    it('should handle empty security findings', () => {
      type SecurityFindingMinimal = { type: string; severity: string; };
      const findings: SecurityFindingMinimal[] = [];
