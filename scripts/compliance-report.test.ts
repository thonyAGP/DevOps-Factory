...
// Minimal interfaces for specific test cases to resolve 'unknown' types
interface MinimalPRForReviewCoverage {
  reviewers?: string[];
}

interface MinimalRepoForEmptyList {
  mergedPRs?: any[];
  score?: number;
}

interface MinimalSecurityFinding {
  repo?: string;
  type: string;
  severity: string;
  count?: number;
...
describe('compliance-report', () => {
  ...
  describe('PR Review Coverage', () => {
    ...
    it('should handle empty PR list', () => {
      const prs: MinimalPRForReviewCoverage[] = [];
...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      const repos: MinimalRepoForEmptyList[] = [];
