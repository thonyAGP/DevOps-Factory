...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      interface RepoWithMetrics {
        mergedPRs?: any[]; // Only length is accessed, so any[] is sufficient
        score: number;
      }
  ...
  describe('Security Findings Aggregation', () => {
    interface SecurityFinding {
      repo?: string; // Not strictly needed for the error, but good for context
      type: string;
      severity: string;
      count?: number;
      lastScan?: string;
