...
interface Feedback {
  ...
  source: 'self-heal' | 'test-scaffold' | 'ai-test-writer' | 'prettier-fix' | 'unknown';
}
...
const detectSource = (labels: string[], title: string): Feedback['source'] => {
  if (labels.includes('ai-fix') || title.includes('AI-generated') || title.includes('CI fix'))
    return 'self-heal';
  if (title.includes('test scaffold') || title.includes('Test scaffold')) return 'test-scaffold';
  if (title.includes('AI tests') || title.includes('ai-tests')) return 'ai-test-writer';
...
const detectPatternId = (title: string, labels: string[]): string | null => {
  if (!labels.includes('ai-fix')) return null;

  // Check title: fix: CI fix [pattern:xxx]
  const titleMatch = title.match(/\[pattern:([^\]]+)\]/);
  if (titleMatch) return titleMatch[1];

  // Legacy format: pattern:xxx without brackets
  const legacyMatch = title.match(/pattern:(\S+)/);
...
const getAutoGeneratedPRs = (repo: string): PRInfo[] => {
  const result = sh(
    `gh pr list --repo ${repo} --state all --limit 20 --json number,title,state,labels,createdAt,closedAt,mergedAt,commits --jq "[.[] | select(.title | test(\\"DevOps-Factory|AI-generated|CI fix|AI tests|ai-tests|test scaffold|auto-fix|prettier\\"; \\"i\\")) | {number, title, state, merged: (.mergedAt != null), labels: [.labels[].name], createdAt, closedAt, mergedAt, commits}]"`
  );
  try {
