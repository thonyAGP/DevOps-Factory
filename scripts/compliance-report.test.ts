...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      interface TestRepo { mergedPRs?: { number: number }[]; score?: number; }
  ...
  describe('Security Findings Aggregation', () => {
    it('should aggregate findings by severity', () => {
      interface TestSecurityFinding { repo: string; type: string; severity: string; count: number; lastScan: string; }
    ...
    it('should handle empty security findings', () => {
      interface TestSecurityFinding { type: string; severity: string; }
      const findings: TestSecurityFinding[] = [];
