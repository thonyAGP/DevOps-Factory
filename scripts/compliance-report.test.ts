...
describe('compliance-report', () => {
  ...
  describe('PR Review Coverage', () => {
    ...
    it('should handle empty PR list', () => {
      const prs: { reviewers: string[] }[] = []; // Fix: specify type for 'pr'
...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      const repos: { mergedPRs?: { number: number }[]; score: number }[] = []; // Fix: specify type for 'r'
  ...
  describe('Security Findings Aggregation', () => {
    it('should aggregate findings by severity', () => {
      const findings: { type: string; severity: string }[] = [ // Fix: specify type for 'finding'
