...
const createConfigPR = (analysis: RepoAnalysis): void => {
  ...
  if (!baseSha) {
    ...
    logActivity('scan-and-configure', 'error', 'Cannot get base SHA', 'error', repo.name);
    return;
  }
...
> Auto-generated by [DevOps-Factory](https://github.com/thonyAGP/DevOps-Factory)"`
  ...
  const fileNames = filesToAdd.map((f) => f.path.split('/').pop()).join(', ');
  logActivity(
    'scan-and-configure',
    'pr-created',
    `Added ${filesToAdd.length} workflow(s): ${fileNames}`,
    'success',
    repo.name
  );
};
...
const main = () => {
  ...
  const activeCount = analyses.filter((a) => a.stack !== 'unknown').length;
  logActivity(
    'scan-and-configure',
    'scan-complete',
    `Scanned ${scannedCount} repos (${activeCount} active)`,
    'success'
  );
