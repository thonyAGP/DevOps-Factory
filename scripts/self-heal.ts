...
// --- Circuit Breaker: prevent heal storms on a single repo ---

const MAX_OPEN_HEALING_PRS = 3;

/**
 * Check if a repo already has too many open healing PRs.
 * If MAX_OPEN_HEALING_PRS or more ai-fix PRs are open, pause self-healing.
 */
const isCircuitBreakerOpen = (repo: string): boolean => {
  try {
    const raw = sh(
...
const main = async (): Promise<void> => {
  ...
  // 0c. Circuit breaker — too many open healing PRs on this repo
  ...
  // 0d. Flaky test detection - re-run failed jobs instead of fixing
  ...
  if (
    ...
    // Step 4b: Check knowledge graph for a validated fix (skip LLM if found)
    if (usedPatternId) {
      ...
      if (knownFix) {
        ...
        // Apply the known fix directly — parse diff into replacements
        ...
        // Feed the known diff as a pattern hint for the AI to refine
    ...
    // Step 4c: Send to AI for code generation (pattern hint guides the fix)
