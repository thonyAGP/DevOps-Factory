...
describe('compliance-report', () => {
  ...
  describe('PR Review Coverage', () => {
    ...
    it('should handle empty PR list', () => {
      interface MergedPRForCoverage {
        reviewers?: string[];
      }
      const prs: MergedPRForCoverage[] = [];
...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      interface RepoForEdgeCase {
        mergedPRs?: { length: number }[];
        score: number;
      }
      const repos: RepoForEdgeCase[] = [];
  ...
  describe('Security Findings Aggregation', () => {
    it('should aggregate findings by severity', () => {
      interface SecurityFindingForAggregation {
        repo: string;
        type: string;
        severity: string;
        count: number;
      ...
      const findings: SecurityFindingForAggregation[] = [
