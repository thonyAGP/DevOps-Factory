...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      const repos: { mergedPRs?: MergedPR[]; score?: number; }[] = []; // Fix: type repos array

      const totalPRs = repos.reduce((s, r) => s + (r.mergedPRs?.length || 0), 0);
      const avgScore = repos.length > 0 ? repos.reduce((s, r) => s + (r.score || 0), 0) / repos.length : 0; // Fix: handle undefined score
