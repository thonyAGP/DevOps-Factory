...
const detectSource = (labels: string[], title: string): Feedback['source'] => {
  if (labels.includes('ai-fix') || title.includes('') || title.includes('CI fix'))
    return 'self-heal';
...
const getAutoGeneratedPRs = (repo: string): PRInfo[] => {
  const result = sh(
    `gh pr list --repo ${repo} --state all --limit 20 --json number,title,state,labels,createdAt,closedAt,mergedAt,commits --jq "[.[] | select(.title | test(\\"DevOps-Factory||CI fix|AI tests|ai-tests|test scaffold|auto-fix|prettier\\"; \\"i\\")) | {number, title, state, merged: (.mergedAt != null), labels: [.labels[].name], createdAt, closedAt, mergedAt, commits}]"`
  );
  try {
