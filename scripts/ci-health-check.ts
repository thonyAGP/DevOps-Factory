...
const createIssue = (_factoryRepo: string, result: CICheckResult): void => {
  const { project, run } = result;
  if (!run) return;

  // Log locally instead of creating individual GitHub issues (daily digest handles notifications)
  console.log(`  [CI-FAIL] ${project.name}: ${run.name} on ${run.head_branch} (${run.html_url})`);
  logActivity(
    'ci-health-check',
    'ci-failure-detected',
...
const trackFailure = (_factoryRepo: string, result: CICheckResult): void => {
  ...
  if (entry.consecutiveFailures >= ESCALATION_THRESHOLD && !entry.escalated) {
    // Log escalation instead of creating issue (daily digest handles notifications)
    console.log(
...
const triggerSelfHeal = (factoryRepo: string, result: CICheckResult): void => {
  ...
  // Skip paused repos
  if (project.healingState === 'paused') {
    console.log(`  [SKIP HEAL] ${project.name}: healingState is "paused"`);
    return;
  }
