...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      type RepoItem = { mergedPRs?: MergedPR[] } | { score?: number };
      const repos: RepoItem[] = [];

      const totalPRs = repos.reduce((s, r) => s + ((r as RepoWithMergedPRs).mergedPRs?.length || 0), 0);
      const avgScore = repos.length > 0 ? repos.reduce((s, r) => s + ((r as RepoWithScore).score || 0), 0) / repos.length : 0;
