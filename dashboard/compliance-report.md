# Compliance & Audit Report

**Generated**: 3/24/2026, 1:11:21 AM
**Period**: 30-day window

## Summary

| Metric | Value |
|--------|-------|
| Total Repos | 19 |
| Avg Compliance Score | 80/100 |
| Repos with Branch Protection | 0/19 |
| Repos with CI | 19/19 |
| PR Review Coverage | 0% |
| PRs Merged (30d) | 92 |
| Deployments (30d) | 147 |
| PRs with Review | 0/92 |

## Repository Compliance

| Repo | Score | Branch Prot | CI | Review | Security |
|------|-------|-------------|----|------------|----------|
| au-marais | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| CasaSync | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| claude-cli-wrapper | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| claude-launcher | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| ClubMedRoomAssignment | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| Email_Assistant | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| LB2I-Fiscal-Manager | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| Livret_accueil_Au-Marais | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| livret-au-marais | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| magic-migration | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| MCP_Quota_Claude | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| Site_1970_Plomberie | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| Site_Greg-Assainissement | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| SqlConnectionTest | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| statusline | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| test_codingmenace | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| Thumbfast | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| Utilitaire_Webapp | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |
| zentra | 🟢 80/100 | ✗ | ✓ | ✗ | 0 |

## Compliance Gaps

### ⚠️ No Branch Protection (19)
- au-marais
- CasaSync
- claude-cli-wrapper
- claude-launcher
- ClubMedRoomAssignment
- Email_Assistant
- LB2I-Fiscal-Manager
- Livret_accueil_Au-Marais
- livret-au-marais
- magic-migration
- MCP_Quota_Claude
- Site_1970_Plomberie
- Site_Greg-Assainissement
- SqlConnectionTest
- statusline
- test_codingmenace
- Thumbfast
- Utilitaire_Webapp
- zentra

### ⚠️ No Code Review (19)
- **au-marais**: 5 PRs merged without review
- **CasaSync**: 5 PRs merged without review
- **claude-cli-wrapper**: 6 PRs merged without review
- **claude-launcher**: 5 PRs merged without review
- **ClubMedRoomAssignment**: 5 PRs merged without review
- **Email_Assistant**: 4 PRs merged without review
- **LB2I-Fiscal-Manager**: 4 PRs merged without review
- **Livret_accueil_Au-Marais**: 5 PRs merged without review
- **livret-au-marais**: 5 PRs merged without review
- **magic-migration**: 6 PRs merged without review
- **MCP_Quota_Claude**: 6 PRs merged without review
- **Site_1970_Plomberie**: 5 PRs merged without review
- **Site_Greg-Assainissement**: 6 PRs merged without review
- **SqlConnectionTest**: 5 PRs merged without review
- **statusline**: 4 PRs merged without review
- **test_codingmenace**: 3 PRs merged without review
- **Thumbfast**: 4 PRs merged without review
- **Utilitaire_Webapp**: 4 PRs merged without review
- **zentra**: 5 PRs merged without review

## Recommendations

1. **Enable Branch Protection** on all repos to enforce code review
2. **Setup CI/CD pipelines** for repos without automated testing
3. **Require code reviews** before merge on default branches
4. **Enable security scanning** (dependabot, code scanning)
5. **Monitor review coverage** - target 100% reviewed PRs

## Detailed Repository Breakdown

### au-marais | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/au-marais`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #12 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #11 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #10 - chore: add security workflows (gitleaks, semgrep, license, supply-chain) (by thonyAGP)
- #9 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #8 - Admin improvements: calendar, reservations & mobile fixes (by thonyAGP)

**Recent Deployments**: 8 successful deploys in 30 days

---

### CasaSync | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/CasaSync`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #80 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #79 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #77 - chore: add security workflows (gitleaks, semgrep, license, supply-chain) (by thonyAGP)
- #76 - test: add skeleton test files (by thonyAGP)
- #75 - chore: add DevOps-Factory AI workflows (by thonyAGP)

**Recent Deployments**: 13 successful deploys in 30 days

---

### claude-cli-wrapper | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/claude-cli-wrapper`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/6 PRs (0%)

**PRs Without Review**:
- #7 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add SAST with Semgrep (by thonyAGP)
- #4 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #3 - chore: add secret scanning with Gitleaks (by thonyAGP)
- #2 - test: add skeleton test files (by thonyAGP)

**Recent Deployments**: 6 successful deploys in 30 days

---

### claude-launcher | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/claude-launcher`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #4 - chore: add SAST with Semgrep (by thonyAGP)
- #3 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #2 - chore: add secret scanning with Gitleaks (by thonyAGP)

**Recent Deployments**: 13 successful deploys in 30 days

---

### ClubMedRoomAssignment | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/ClubMedRoomAssignment`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #2806 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #2805 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #2798 - style: auto-fix Prettier formatting (122 files) (by thonyAGP)
- #2786 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #2780 - fix(ci): add pnpm version to action-setup (by thonyAGP)

**Recent Deployments**: 6 successful deploys in 30 days

---

### Email_Assistant | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/Email_Assistant`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/4 PRs (0%)

**PRs Without Review**:
- #2799 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #2798 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #2796 - chore: add security workflows (gitleaks, semgrep, license, supply-chain) (by thonyAGP)
- #2782 - chore: add DevOps-Factory AI workflows (by thonyAGP)

**Recent Deployments**: 10 successful deploys in 30 days

---

### LB2I-Fiscal-Manager | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/LB2I-Fiscal-Manager`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/4 PRs (0%)

**PRs Without Review**:
- #4 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #3 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #2 - chore: add security workflows (gitleaks, semgrep, license, supply-chain) (by thonyAGP)
- #1 - chore: add DevOps-Factory AI workflows (by thonyAGP)

**Recent Deployments**: 4 successful deploys in 30 days

---

### Livret_accueil_Au-Marais | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/Livret_accueil_Au-Marais`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #4 - chore: add SAST with Semgrep (by thonyAGP)
- #3 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #2 - chore: add secret scanning with Gitleaks (by thonyAGP)

**Recent Deployments**: 10 successful deploys in 30 days

---

### livret-au-marais | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/livret-au-marais`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #7 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add security workflows (gitleaks, semgrep, license, supply-chain) (by thonyAGP)
- #3 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #2 - chore: add secret scanning with Gitleaks (by thonyAGP)

**Recent Deployments**: 9 successful deploys in 30 days

---

### magic-migration | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/magic-migration`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/6 PRs (0%)

**PRs Without Review**:
- #26 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #25 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #24 - chore: add security workflows (gitleaks, semgrep, license, supply-chain) (by thonyAGP)
- #21 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #20 - chore: add secret scanning with Gitleaks (by thonyAGP)
- #10 - feat(qa): QA Session Complete - Version Badge + Security Fixes (by thonyAGP)

**Recent Deployments**: 14 successful deploys in 30 days

---

### MCP_Quota_Claude | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/MCP_Quota_Claude`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/6 PRs (0%)

**PRs Without Review**:
- #7 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add SAST with Semgrep (by thonyAGP)
- #4 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #3 - chore: add secret scanning with Gitleaks (by thonyAGP)
- #2 - test: add skeleton test files (by thonyAGP)

**Recent Deployments**: 10 successful deploys in 30 days

---

### Site_1970_Plomberie | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/Site_1970_Plomberie`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #4 - chore: add SAST with Semgrep (by thonyAGP)
- #3 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #2 - chore: add secret scanning with Gitleaks (by thonyAGP)

**Recent Deployments**: 4 successful deploys in 30 days

---

### Site_Greg-Assainissement | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/Site_Greg-Assainissement`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/6 PRs (0%)

**PRs Without Review**:
- #7 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add SAST with Semgrep (by thonyAGP)
- #4 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #3 - chore: add secret scanning with Gitleaks (by thonyAGP)
- #2 - test: add skeleton test files (by thonyAGP)

**Recent Deployments**: 2 successful deploys in 30 days

---

### SqlConnectionTest | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/SqlConnectionTest`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #4 - chore: add SAST with Semgrep (by thonyAGP)
- #3 - chore: add automated dependency updates with Renovate (by thonyAGP)
- #2 - chore: add secret scanning with Gitleaks (by thonyAGP)

**Recent Deployments**: 1 successful deploys in 30 days

---

### statusline | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/statusline`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/4 PRs (0%)

**PRs Without Review**:
- #9 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #8 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #7 - test: add skeleton test files (by thonyAGP)
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)

---

### test_codingmenace | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/test_codingmenace`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/3 PRs (0%)

**PRs Without Review**:
- #7 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #5 - chore: add DevOps-Factory AI workflows (by thonyAGP)

**Recent Deployments**: 25 successful deploys in 30 days

---

### Thumbfast | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/Thumbfast`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/4 PRs (0%)

**PRs Without Review**:
- #9 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #8 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #7 - test: add skeleton test files (by thonyAGP)
- #6 - chore: add DevOps-Factory AI workflows (by thonyAGP)

**Recent Deployments**: 12 successful deploys in 30 days

---

### Utilitaire_Webapp | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/Utilitaire_Webapp`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/4 PRs (0%)

**PRs Without Review**:
- #13 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #12 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #11 - test: add skeleton test files (by thonyAGP)
- #10 - chore: add DevOps-Factory AI workflows (by thonyAGP)

---

### zentra | ✓ EXCELLENT (80/100)

**Full Name**: `thonyAGP/zentra`

**Controls**:
- Branch Protection: ✗
- Code Review Required: ✗
- CI/CD Enabled: ✓
- Security Findings: 0 alerts

**Review Coverage**: 0/5 PRs (0%)

**PRs Without Review**:
- #43 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #42 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #40 - chore: add security workflows (gitleaks, semgrep, license, supply-chain) (by thonyAGP)
- #35 - chore: add DevOps-Factory AI workflows (by thonyAGP)
- #34 - fix(ci): add pnpm version to action-setup (by thonyAGP)

---
