...
2. **Setup CI/CD pipelines** for repos without automated testing`;
  ...
  describe('Edge Cases', () => {
    it('should handle empty repos list', () => {
      interface RepoWithPRs { mergedPRs?: { number: number }[]; }
      interface RepoWithScore { score?: number; }
      const repos: (RepoWithPRs & RepoWithScore)[] = [];
  ...
  describe('Security Findings Aggregation', () => {
    ...
    it('should handle empty security findings', () => {
      interface SecurityFinding { type: string; severity: string; count: number; repo: string; lastScan: string; }
